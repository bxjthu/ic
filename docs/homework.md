...
**注意：同学们Fork和clone出来的代码库都需要与主库保持同步，因此除了提交作业文件和更新作业文件，都不要改动仓库里的任何代码文件，包括添加、删除、更改自己或他人的代码文件。**

## 9. 更新作业

在作业提交截止时间之前，同学们可以任意更改更新作业。具体步骤与前述相同，但只需从第5步开始。

另外，如果之前提交的Pull request还没有被助教通过，那更新(也就是更新到自己的服务器库)之后会自动更新到原来的Pull request里，不需要再提交新的pull request。但如果之前的pull request已经通过或关闭，那就需要再次提交新的pull requests。