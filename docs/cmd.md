...
1. 目标路径名通常省略，即显示当前目录内容
2. ls的参数很多，可以使用man ls命令查看每个参数的含义
3. 常用的几个参数 -G 带颜色显示（默认不带颜色） -A 显示除了.和..的所有内容（某些内容默认不显示）-F 根据每项类型不同添加不同符号
...
1. 路径分为绝对路径和相对路径两种，在mac的系统osX和Linux中，以/开头的路径格式是绝对路径（如/opt/docklet)，需要从根目录一级一级寻找，不以/开头的路径是相对路径（如Desktop/a.txt)，从当前目录开始一级一级寻找。
2. 对于每个目录而言，.指的是它自身，..指的是它的上一级目录，在ls中这两个项默认不显示
3. 如果要运行可执行程序，即使是在当前目录下也要加上./以区别于系统中的其他命令或程序
4. Linux/osX中的目录分隔符/和Windows方向相反
...
如果使用通配符，指定路径下有匹配该通配符的文件夹时，会同时显示该文件夹下所有内容
...
使用说明：
1. 目标路径2】以/结束的话复制到应文件夹，不以/结束的话会将用最后一个/之后的内容命名复制之后的内容
2. 可以使用通配符复制多个文件，此时目标路径不加/会报错
3. 某些情况出错不会报错（如目标路径下已经存在同名文件夹）
4. 复制文件夹要添加-r参数
...
使用说明：
1. 参见cp命令，但是移动目录不需要添加-r参数
2. -f 强制移动（覆盖不需要确认） -i 覆盖之前确认 -n 不覆盖 -v 移动完成后显示移动后的文件
